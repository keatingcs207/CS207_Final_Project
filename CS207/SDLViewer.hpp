...
class SDLViewer {
 ...
 private:
  ...
  void handle_event(SDL_Event event) {
    ...
    // Loop through our listeners to trigger 
    for (unsigned int i=0; i<listeners_.size(); ++i) {
      EventListener* handler=listeners_[i]; 
 ...
 public:
  /**
    * EventListener
    * Representation is an SDL_Event and a functor to execute on trigger 
    * SDL_Events are represented by a keyword string and are available as follows:
    *	MouseDown, RightClick, LeftClick, KeyDown, LongClick
    */
  struct EventListener {

    virtual void operator()() {};

    // LongClick: Passes through the duration of the click in seconds
    virtual void operator()(double i) {(void)i;};	

    // KeyDown: Passes through the key that was pressed
    virtual void operator()(const char* key) {(void)key;};

    // Set the start time for the longclick event
    void start_timer() {
      start_time_ = clock();
    }

    // Get the current duration of the longclick event
    double get_timer() {
      return double(clock()-start_time_)/CLOCKS_PER_SEC;
    }

    // Get the final duration of the longclick event
    double stop_timer() {
      return double(clock()-start_time_)/CLOCKS_PER_SEC;
    }

    /** Function to fire the event 
      * Convert our event types to their corresponding SDL events
      * Fire the event if the event type matches even.type
      */
    void fire_event(SDL_Event event) {
      switch (event.type) {
        // "MouseDown" event
        case SDL_MOUSEBUTTONDOWN : {
          if (strcmp(type_, "MouseDown") == 0) (*this)();
	  // "LeftClick" event
          if (event.button.button == SDL_BUTTON_LEFT) {
            if (strcmp(type_, "LeftClick") == 0) (*this)();
	    // Setup the "LongClick" event
            if (strcmp(type_, "LongClick") == 0) this->start_timer();
          }
	  // "RightClick" event
          if (event.button.button == SDL_BUTTON_RIGHT)
            if (strcmp(type_, "RightClick") == 0) (*this)();

        } break;

        // "KeyDown event
        case SDL_KEYDOWN: {
          if (strcmp(type_, "KeyDown") == 0) 
            (*this)(SDL_GetKeyName(event.key.keysym.sym));
        } break;

        // Fire the "LongClick" event
        case SDL_MOUSEBUTTONUP : {
          if (event.button.button == SDL_BUTTON_LEFT)
            if (strcmp(type_, "LongClick") == 0) 
              (*this)(this->stop_timer());
        } break;
      } 
    }

    /** Sets the event type
      * called from add_listener
      */
   ...
   private:
    const char* type_;		// String representing the SDL_Event type
    clock_t start_time_;	// Start for the LongClick event

  };

  // Push the event listener to listeners_
  // Add the type that will trigger the event
  void add_listener(const char* event_type, EventListener* listener) {
    listener->set_type(event_type);

    // Push the event into our vector of listeners
